...
If there exists $p\in\Naturals$, $A_{i}\in\lbrace W,X,Y,Z\rbrace$ and $t_{i}>0$, for $i\in\lbrace 1,\ldots,p\rbrace$,
...
Formally, there exists $i$ and $j$ such that $A_{i}=X$ and $A_{j}=Y$, and for
...
\end{tikzpicture}
\end{center}
Starting from the identity and applying the different products $e^{A_{i}t_{i}}$ in the diagram,
...
that $A_{i}=Y$ and $A_{j}=X$ then by the diagram, we would end up with a matrix where
the top right corner in nonzero, which contradicts $G=\prod_{i=1}^{p} e^{A_{i} t_{i}}$.
\end{proof}
...
But since the $B_{i}$ are the only one to contribute to $V$, $V$ must be of the form:
