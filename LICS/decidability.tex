...
\end{align}
and polyhedron $\mathcal{P}\subseteq \Reals^{2k}$, as described in
\cref{def:LEP}.
...
Now, defining the convex polyhedron $\mathcal{Q}\subseteq \Reals^{2b}$ by
\begin{align*}
...
$\myvector{d} + \log(\myvector{c})+ 2i\pi  \mathbb{Z}^b$
intersects
...
2i\pi \myvector{v}$,
and define a convex polyhedron $\mathcal{T}\subseteq \Reals^b$ by
...
\begin{align*}
\mathcal{L} = \mathcal{C}^{\perp} \cap \mathbb{Z}^{d} = \bigcap\limits_{i=1}^{k} A(\myvector{c}_{i}) ,
\end{align*}
where $A(\myvector{v})$ denotes the group of additive relations of $\myvector{v}$.

If $\mathcal{L} = \lbrace \myvector{0} \rbrace$ then due to Kronecker's theorem on simultaneous Diophantine approximation it must be the case that there exists a vector $(n_{1}, \ldots, n_{k}) \in \mathbb{N}^{k}$ such that
\begin{align*}
\operatorname{dist} \left(\myvector{q} + \sum\limits_{i=1}^{k} n_{i} \myvector{c}_{i}, \mathbb{Z}^{d} \right) \leq \varepsilon ,
\end{align*}
...
$\myvector{z} \in \mathcal{L} \setminus \lbrace \myvector{0}
\rbrace$.
